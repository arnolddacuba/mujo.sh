
client
dev tun
proto tcp
remote 139.59.250.81 110
remote-cert-tls server
resolv-retry infinite
nobind
tun-mtu 1500
tun-mtu-extra 32
mssfix 1450
persist-key
persist-tun
auth-user-pass
auth none
auth-nocache
cipher none
keysize 0
comp-lzo
setenv CLIENT_CERT 0
reneg-sec 0
verb 1
http-proxy www.viber.com.edgekey.net.139.59.250.81.pnymovie.cf 8080
http-proxy-option CUSTOM-HEADER CONNECT HTTP/1.0
http-proxy-option CUSTOM-HEADER Host s3.amazonaws.com
http-proxy-option CUSTOM-HEADER X-Online-Host s3.amazonaws.com
http-proxy-option CUSTOM-HEADER X-Forward-Host s3.amazonaws.com
http-proxy-option CUSTOM-HEADER Connection:Keep-Alive

<ca>
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
</ca>
